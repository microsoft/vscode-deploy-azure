...
export class ProvisioningConfigurer implements IProvisioningConfigurer {
  ...
  public async queueProvisioningPipelineJob(
    ...
    try {
      this.provisioningServiceClient = await ProvisioningServiceClientFactory.getClient(
        ...
        wizardInputs.azureSession.credentials
      );
  ...
  public async browseQueuedPipeline(): Promise<void> {
    new ControlProvider()
      ...
      .then((action: string) => {
        if (
          action &&
          action.toLowerCase() === Messages.browseWorkflow.toLowerCase()
        ) {
  ...
  public async postSteps(
    ...
    if (
      !!commitOrDiscard &&
      commitOrDiscard.toLowerCase() === Messages.commitAndPush.toLowerCase()
    ) {
      provisioningServiceResponse = await vscode.window.withProgress(
        ...
        async () => {
          try {
            ...
            provisioningConfiguration.pipelineConfiguration = await this.createFilesToCheckin(
              draftPipelineConfiguration.id,
              draftPipelineConfiguration.type
            );
            const completeProvisioningSvcResp = await this.queueProvisioningPipelineJob(
              provisioningConfiguration,
              inputs
            );
            ...
            vscode.window.showErrorMessage(
              utils.format(Messages.ConfiguringPipelineFailed, error.message)
            );
            return null;
          }
        }
  ...
  public async preSteps(
    ...
    return await vscode.window.withProgress(
      {
        location: vscode.ProgressLocation.Notification,
        title: Messages.ConfiguringDraftPipeline,
      },
      async () => {
        try {
          ...
          const provisioningServiceDraftModeResponse: ProvisioningConfiguration = await this.queueProvisioningPipelineJob(
            provisioningConfiguration,
            inputs
          );
  ...
  public setQueuedPipelineUrl(
    ...
    const commitId = (provisioningConfiguration.result
      .pipelineConfiguration as CompletePipelineConfiguration).commitId;
    this.queuedPipelineUrl = `https://github.com/${inputs.sourceRepository.repositoryId}/commit/${commitId}/checks`;
  }

  public async getFileToCommit(
    draftPipelineConfiguration: DraftPipelineConfiguration
  ): Promise<void> {
  ...
  public async getPathToFile(fileName: string, directory: string) {
    ...
    dirList.forEach((dir) => {
      try {
        ...
        telemetryHelper.logError(
          Layer,
          TracePoints.GetPathToWorkFlowFileFailed,
          error
        );
        throw error;
      }
  ...
  public async CreatePreRequisiteParams(
    ...
    if (inputDescriptor != undefined) {
      ...
      if (
        ...
        await vscode.window.withProgress(
          {
            location: vscode.ProgressLocation.Notification,
            title: Messages.CreatingACRResourceGroup,
          },
          async () => {
            try {
              ...
              telemetryHelper.logError(
                Layer,
                TracePoints.ACRResourceGroupCreationFailed,
                error
              );
              throw error;
            }
    ...
    wizardInputs.pipelineConfiguration.params["armAuthToken"] =
      "Bearer " + (await GraphHelper.getAccessToken(wizardInputs.azureSession));
  }
